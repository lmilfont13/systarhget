...
(async () => {
  ...
  // Select text template (usually Atacadão is the second or third, let's find one)
  await page.evaluate(() => {
     const select = document.querySelector('select');
     const optIndex = Array.from(select.options).findIndex(o => o.innerText.includes('[Texto]'));
     if(optIndex > 0) {
        select.value = select.options[optIndex].value;
        select.dispatchEvent(new Event('change', { bubbles: true }));
        console.log('Selected template:', select.options[optIndex].innerText);
     } else {
        console.log('No text template found!');
     }
  });
